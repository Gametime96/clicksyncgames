...
// --- GAME STATE MANAGER & RULES ---
let gameState = 'MENU'; 
let gameMode = 'PvP';   
let aiDifficulty = 'Mediocre'; // 'Beginner', 'Mediocre', 'Expert'
let currentPlayer = 1;  
let shotActive = false;
let turnTimerTriggered = false;
let aiThinking = false;

// Suite Assignment & Fouls
let isBreakShot = true;  // Tracks if this is the first shot of the game
let player1Suite = null; // 'solid' or 'stripe'
let player2Suite = null; 
let firstBallHit = null; // Tracks the first ball struck by the cue ball
let pocketedThisTurn = []; // Tracks balls sunk during the current shot
...
class Ball {
    ...
    draw() {
        ...
        let drawPoleDetails = (poleX, poleY, poleZ, isFront) => {
            if (poleZ > 0) {
                let drawX = poleX * BALL_RADIUS;
                let drawY = poleY * BALL_RADIUS;

                if (this.type === 'cue') {
                    if (isFront) { 
                        ctx.beginPath();
                        ctx.arc(drawX, drawY, BALL_RADIUS * 0.25, 0, Math.PI * 2);
                        ctx.fillStyle = '#dc2626';
                        ctx.fill();
                    }
                } else {
                    ctx.beginPath();
                    ctx.arc(drawX, drawY, BALL_RADIUS * 0.55, 0, Math.PI * 2);
    ...
    update() {
        ...
        if (this.state === 'chute_drop') {
            ...
            if (this.y >= 550) { 
                this.y = 550;
                this.state = 'chute_curve';
                this.chuteAngle = Math.PI; 
            }
            return;
        }
        ...
        if (this.state === 'chute_curve') {
            ...
            if (this.chuteAngle <= Math.PI / 2) {
                ...
                this.x = 240 + 40 * Math.cos(this.chuteAngle);
                this.y = 550 + 40 * Math.sin(this.chuteAngle); 
            }
            this.update3DSpin(2, 0);
            return;
        }
        ...
        if (this.state === 'tube') {
            ...
            if (this.x + BALL_RADIUS > limitX) {
                this.x = limitX - BALL_RADIUS;
            }
            
            let distanceMoved = this.x - previousX;
            if (distanceMoved > 0) {
                this.update3DSpin(distanceMoved, 0);
            }
        ...
        if (this.x - BALL_RADIUS < INNER_LEFT + CUSHION_WIDTH) {
            this.x = INNER_LEFT + CUSHION_WIDTH + BALL_RADIUS;
            this.vx = -this.vx * BOUNCE;
        } else if (this.x + BALL_RADIUS > INNER_RIGHT - CUSHION_WIDTH) {
            this.x = INNER_RIGHT - CUSHION_WIDTH - BALL_RADIUS;
            this.vx = -this.vx * BOUNCE;
        }

        if (this.y - BALL_RADIUS < INNER_TOP + CUSHION_WIDTH) {
            this.y = INNER_TOP + CUSHION_WIDTH + BALL_RADIUS;
            this.vy = -this.vy * BOUNCE;
        } else if (this.y + BALL_RADIUS > INNER_BOTTOM - CUSHION_WIDTH) {
            this.y = INNER_BOTTOM - CUSHION_WIDTH - BALL_RADIUS;
            this.vy = -this.vy * BOUNCE;
        }

        for (let pocket of pockets) {
            const distance = Math.hypot(this.x - pocket.x, this.y - pocket.y);
            
            if (distance < POCKET_RADIUS && this.state === 'active') {
                this.state = 'pocketed';
                this.vx = 0;
                this.vy = 0;
                ...
                if (this.type !== 'cue' && this.type !== '8ball') {
                    setTimeout(() => {
                        this.state = 'chute_drop';
                        this.x = 200; 
                        this.y = TABLE_Y + TABLE_HEIGHT; 
                    }, 2000);
                }
            }
        }
    ...
    for (let i = 0; i < balls.length; i++) {
        for (let j = i + 1; j < balls.length; j++) {
            ...
            if (distance < minAllowedDistance) {
                ...
                b0.x -= nx * (overlap * ratio0);
                b0.y -= ny * (overlap * ratio0);
                ...
                const v0x = b0.vx * cos + b0.vy * sin;
                const v0y = b0.vy * cos - b0.vx * sin;
                const v1x = b1.vx * cos + b1.vy * sin;
                const v1y = b1.vy * cos - b1.vx * sin;

                const finalV0x = v1x * BOUNCE;
                const finalV1x = v0x * BOUNCE;

                b0.vx = finalV0x * cos - v0y * sin;
                b0.vy = v0y * cos + finalV0x * sin;
...
function areBallsMoving() {
    for (let b of balls) {
        if (b.state === 'active' && (Math.abs(b.vx) > 0 || Math.abs(b.vy) > 0)) return true;
    }
    return false;
}
...
function evaluateShot() {
    ...
    if (eightBallPocketed) {
        if (isBreakShot) {
            alert(`WOW! ${playerStr} sunk the 8-Ball on the break and WINS THE GAME!`);
            window.location.reload();
            return;
        } else {
            let myBallsLeft = mySuite ? balls.filter(b => b.type === mySuite && b.state === 'active').length : 15;
            if (myBallsLeft === 0 && !foul) {
                alert(`YOU WIN! ${playerStr} legally sunk the 8-Ball.`);
            } else {
                alert(`GAME OVER! ${playerStr} illegally sunk the 8-Ball.`);
            }
            window.location.reload();
            return;
        }
    ...
    if (!firstBallHit) {
        ...
        if (mySuite) {
            ...
            if (firstBallHit.type === '8ball') {
                let myBallsLeft = balls.filter(b => b.type === mySuite && b.state === 'active').length;
                if (myBallsLeft > 0) foul = true;
            }
        } else {
    ...
    for (let b of pocketedThisTurn) {
        if (b.type !== 'cue' && b.type !== '8ball') {
            if (!mySuite && !foul) {
                if (currentPlayer === 1) {
                    player1Suite = b.type;
                    player2Suite = b.type === 'solid' ? 'stripe' : 'solid';
                } else {
                    player2Suite = b.type;
                    player1Suite = b.type === 'solid' ? 'stripe' : 'solid';
                }
                mySuite = b.type;
                newlyAssigned = true;
                turnContinues = true;
            } else if (b.type === mySuite && !foul) {
                turnContinues = true;
            }
        }
    }
    ...
    if (foul) {
        turnContinues = false;
        transientMessage = "FOUL! Opponent gets Cue Ball in Hand.";
        messageTimer = 180; 
        
        cueBall.state = 'active';
        cueBall.x = CUE_START_X;
        cueBall.y = CUE_START_Y;
        cueBall.vx = 0; cueBall.vy = 0;
    } else if (newlyAssigned) {
        transientMessage = `${playerStr} is now ${mySuite.toUpperCase()}S!`;
        messageTimer = 150;
    } else if (!turnContinues) {
        transientMessage = "Turn Ends.";
        messageTimer = 90;
    } else {
        transientMessage = "Nice Shot! Keep going.";
        messageTimer = 90;
    }
    ...
    if (gameMode === 'PvC' && currentPlayer === 2) {
        aiThinking = true;
        setTimeout(takeComputerTurn, 1500);
    }
}
...
function takeComputerTurn() {
    ...
    let mySuite = player2Suite;
    let targets = [];
    
    if (!mySuite) {
        targets = balls.filter(b => (b.type === 'solid' || b.type === 'stripe') && b.state === 'active');
    } else {
        targets = balls.filter(b => b.type === mySuite && b.state === 'active');
    }
    
    if (targets.length === 0) {
        targets = balls.filter(b => b.type === '8ball' && b.state === 'active');
    }
    ...
    for (let t of targets) {
        for (let p of pockets) {
            let distToPocket = Math.hypot(t.x - p.x, t.y - p.y);
            let distToTarget = Math.hypot(t.x - cueBall.x, t.y - cueBall.y);
            let score = distToPocket + (distToTarget * 0.4);

            if (score < minScore) {
                minScore = score;
                bestShot = { target: t, pocket: p };
            }
        }
    }

    if (bestShot) {
        let t = bestShot.target;
        let p = bestShot.pocket;

        let dx = t.x - p.x;
        let dy = t.y - p.y;
        let mag = Math.hypot(dx, dy);

        let gx = t.x + (dx / mag) * (BALL_RADIUS * 2);
        let gy = t.y + (dy / mag) * (BALL_RADIUS * 2);

        let shootDx = gx - cueBall.x;
        let shootDy = gy - cueBall.y;
        ...
        let errorAngle = (Math.random() - 0.5) * difficultyFactor; 
        ...
        cueBall.vx = (finalDx / shootMag) * power;
        cueBall.vy = (finalDy / shootMag) * power;
    } else {
        cueBall.vx = 20;
        cueBall.vy = 5;
    }
    ...
    shotActive = true;
    turnTimerTriggered = false;
    aiThinking = false;
}

// --- TWO-STEP AIMING CONTROLS & POWER METER ---
let aimState = 'idle'; 
let mouseX = 0;
let mouseY = 0;
let lockedAimX = 0;
...
canvas.addEventListener('mousedown', (e) => {
    ...
    if (gameState === 'MENU') {
        if (mouseX > (canvas.width / 2 - 140) && mouseX < (canvas.width / 2 + 140)) {
            if (mouseY > 200 && mouseY < 250) {
                gameMode = 'PvP'; gameState = 'PLAYING';
                cueBall.pole = { x: -1, y: 0, z: 0.1 };
            } else if (mouseY > 270 && mouseY < 320) {
                gameMode = 'PvC'; aiDifficulty = 'Beginner'; gameState = 'PLAYING';
                cueBall.pole = { x: -1, y: 0, z: 0.1 };
            } else if (mouseY > 340 && mouseY < 390) {
                gameMode = 'PvC'; aiDifficulty = 'Mediocre'; gameState = 'PLAYING';
                cueBall.pole = { x: -1, y: 0, z: 0.1 };
            } else if (mouseY > 410 && mouseY < 460) {
                gameMode = 'PvC'; aiDifficulty = 'Expert'; gameState = 'PLAYING';
                cueBall.pole = { x: -1, y: 0, z: 0.1 };
            }
        }
        return;
    ...
    if (gameState === 'PLAYING' && isHumanTurn && cueBall.vx === 0 && cueBall.vy === 0 && cueBall.state === 'active' && !shotActive) {
        if (aimState === 'idle') {
            ...
            const dx = cueBall.x - lockedAimX;
            const dy = cueBall.y - lockedAimY;
            const dist = Math.hypot(dx, dy);
            
            if (dist > 5) {
                cueBall.vx = (dx / dist) * powerLevel;
                cueBall.vy = (dy / dist) * powerLevel;
                shotActive = true;
                turnTimerTriggered = false;
            }
            aimState = 'idle';
...
canvas.addEventListener('mouseup', (e) => {
    if (aimState === 'aiming') {
        const dist = Math.hypot(cueBall.x - mouseX, cueBall.y - mouseY);
        if (dist > 5) {
            aimState = 'power';
            lockedAimX = mouseX;
            lockedAimY = mouseY;
...
function drawMenu() {
    ...
    ctx.textAlign = 'center';
    ctx.fillText('2D BILLIARDS', canvas.width / 2, 130);

    ctx.fillStyle = '#2563eb'; 
    ctx.fillRect(canvas.width / 2 - 140, 200, 280, 50);
    ctx.fillStyle = '#ffffff';
    ctx.font = 'bold 20px sans-serif';
    ctx.fillText('Player 1 vs Player 2', canvas.width / 2, 232);

    ctx.fillStyle = '#16a34a'; 
    ctx.fillRect(canvas.width / 2 - 140, 270, 280, 50);
    ctx.fillStyle = '#ffffff';
    ctx.fillText('PvC - Beginner', canvas.width / 2, 302);

    ctx.fillStyle = '#ea580c'; 
    ctx.fillRect(canvas.width / 2 - 140, 340, 280, 50);
    ctx.fillStyle = '#ffffff';
    ctx.fillText('PvC - Mediocre', canvas.width / 2, 372);

    ctx.fillStyle = '#dc2626'; 
    ctx.fillRect(canvas.width / 2 - 140, 410, 280, 50);
    ctx.fillStyle = '#ffffff';
    ctx.fillText('PvC - Expert', canvas.width / 2, 442);
}
...
function drawUI() {
    ...
    let vsText = gameMode === 'PvC' ? `Computer ${p2Str}` : `Player 2 ${p2Str}`;
    ctx.fillText(`Player 1 ${p1Str} vs ${vsText}`, canvas.width / 2, 25);

    let turnText = "";
    if (currentPlayer === 1) turnText = "Player 1's Turn";
    else if (gameMode === 'PvC') turnText = "Computer is Thinking...";
    else turnText = "Player 2's Turn";
    ...
    if (messageTimer > 0) {
        ...
        ctx.fillText(transientMessage, canvas.width / 2, TABLE_Y + TABLE_HEIGHT + 25);
        messageTimer--;
    }
}
...
function drawTable() {
    ...
    ctx.fillStyle = '#ffffff';
    let markers = [];
    const segment = INNER_WIDTH / 8; 
    
    for (let i = 1; i <= 7; i++) {
        if (i === 4) continue; 
        markers.push([INNER_LEFT + i * segment, TABLE_Y + BORDER / 2]); 
        markers.push([INNER_LEFT + i * segment, INNER_BOTTOM + BORDER / 2]); 
    }
    for (let i = 1; i <= 3; i++) {
        if (i === 2) continue;
        markers.push([TABLE_X + BORDER / 2, INNER_TOP + i * segment * 2]); 
        markers.push([INNER_RIGHT + BORDER / 2, INNER_TOP + i * segment * 2]); 
    }
    ...
    ctx.fillStyle = '#0f5e1e'; 
    ctx.fillRect(INNER_LEFT, INNER_TOP, INNER_WIDTH, INNER_HEIGHT);

    ctx.lineWidth = 1.5;
    ctx.strokeStyle = 'rgba(255, 255, 255, 0.1)';
    for (let i = 1; i <= CUSHION_WIDTH; i+=1.5) {
        ctx.strokeRect(INNER_LEFT + i, INNER_TOP + i, INNER_WIDTH - 2*i, INNER_HEIGHT - 2*i);
    }
...
function drawExternalReturnSystemBase() {
    ...
    ctx.fillRect(0, TABLE_Y + TABLE_HEIGHT + 2, canvas.width, canvas.height - (TABLE_Y + TABLE_HEIGHT));

    ctx.beginPath();
    ctx.moveTo(170, 520); 
    ctx.lineTo(170, 550);
    ctx.arcTo(170, 620, 240, 620, 70); 
    ctx.lineTo(860, 620);
    ctx.lineTo(860, 560);
    ctx.lineTo(240, 560);
    ctx.arcTo(230, 560, 230, 550, 10); 
    ctx.lineTo(230, 520);
    ctx.closePath();
...
function drawAimingTrajectory() {
    ...
    const targetX = aimState === 'power' ? lockedAimX : mouseX;
    const targetY = aimState === 'power' ? lockedAimY : mouseY;

    const aimDx = cueBall.x - targetX;
    const aimDy = cueBall.y - targetY;
...
function drawCueStick() {
    ...
    let pullBack;
    if (aimState === 'aiming') {
        pullBack = Math.min(Math.hypot(targetX - cueBall.x, targetY - cueBall.y), 150); 
    } else {
        pullBack = (powerLevel / MAX_POWER) * 180; 
    }
    ...
    ctx.beginPath();
    ctx.moveTo(BALL_RADIUS + 5 + (pullBack * 0.2), 0);
    ctx.lineTo(BALL_RADIUS + 200 + (pullBack * 0.2), 0);
    ...
    ctx.beginPath();
    ctx.moveTo(BALL_RADIUS + 5 + (pullBack * 0.2), 0);
    ctx.lineTo(BALL_RADIUS + 10 + (pullBack * 0.2), 0);
    ctx.strokeStyle = '#333333';
    ctx.lineWidth = 6;
    ctx.stroke();
...
function drawPowerMeter() {
    if (aimState === 'power') {
        const width = 20;
        const height = 150;
        const x = INNER_LEFT + 20; 
        const y = INNER_BOTTOM - height - 20;
        ...
        const fillHeight = (powerLevel / MAX_POWER) * height;
        const ratio = powerLevel / MAX_POWER;
        ...
        ctx.textAlign = 'center';
        ctx.fillText('MAX', x + width / 2, y - 10);
        ctx.fillText('POWER', x + width / 2, y + height + 16);
    }
}
...
function gameLoop() {
    ...
    if (shotActive && !moving && !turnTimerTriggered) {
    ...
    let sortedBalls = [...balls].sort((a, b) => a.type === 'cue' ? 1 : -1);
    for (let ball of sortedBalls) { ball.draw(); }
    
    drawExternalReturnSystemGlass(); 
    drawCueStick();
    drawPowerMeter();
